...
def f(x, *args):
    ...
    if TYPE == 'POLYNOMIAL':
        ...
        return np.dot(params[0],x) - params[1]
...
def get_root(params, x_dim):
    if TYPE == 'POLYNOMIAL':
        ...
        # sol = np.linalg.solve(params[0],params[1])
        sol = params[1]/params[0]
        if abs(np.asarray(f(sol, params))).sum() < 1e-4:
            valid = True
        else:
            valid = False
...
def draw_params(n_params, x_dim, batch_size):
    ...
    for i in xrange(batch_size):
        ...
        while not valid:
            if TYPE == 'POLYNOMIAL':
                ...
                params=[];
                params.append(np.random.normal(np.zeros([x_dim,x_dim])))
                params.append(np.random.normal(np.zeros(x_dim)))
    ...
    return params_batch, roots_batch
...
for i in xrange(n_iters):
    ...
    # a_inverse = np.linalg.inv(params_[0][0])
    # a_flat = np.ndarray.flatten(params_[0][0])
    # a_inverse_flat = np.ndarray.flatten(a_inverse)
    # params_ = np.concatenate((a_inverse_flat,params_[0][1]))

    a_inverse = 1/params_[0][0]
    params_ = np.asarray([a_inverse,params_[0][1]])

    params_.shape=(1,len(params_))
    ...
    if i % 1000 == 0:
        ...
        iters_vec.append(i)
        print 'Processed %d/%d iters. Loss: %.4f, madad: %.4f, abs_grad: %.2f, abs_w: %.2f' %\
              (i, n_iters, run_vals[1], madad, run_vals[3], run_vals[4])
        plt.pause(0.001)
